import bpy
from .. utils.asset import get_asset_details_from_space, get_asset_ids, get_asset_import_method, get_asset_library_reference, get_libref_and_catalog, get_registered_library_references, set_asset_import_method, set_asset_library_reference
...
class ToggleVIEW3DRegion(bpy.types.Operator):
    ...
    def initiate_asset_browser_area_settings(self, context, debug=False):
        ...
        if context.screen.name not in self.prefs:
            ...
            if 'display_type' not in self.prefs[context.screen.name]['ASSET_TOP']:
                self.prefs[context.screen.name]['ASSET_TOP']['display_type'] = 'THUMBNAIL'

            if 'display_type' not in self.prefs[context.screen.name]['ASSET_BOTTOM']:
                self.prefs[context.screen.name]['ASSET_BOTTOM']['display_type'] = 'THUMBNAIL'
        ...
        for space in area.spaces:
            if space.type == 'FILE_BROWSER':
                if space.params:
                    ...
                    context.scene.M3['asset_browser_prefs'][screen_name][region_type]['display_type'] = space.params.display_type
    ...
    def apply_asset_browser_area_settings(self, context, area, space, params, screen_name, region_type):
        if screen_name in self.prefs:
            ...
            if libref in get_registered_library_references(context):
                ...
                display_type = self.prefs[screen_name][region_type]['display_type']
    ...
    def execute(self, context):
        ...
        for area in context.screen.areas:
            if area.type == 'FILE_BROWSER' and area.ui_type == 'ASSETS':
                ...
                print()
                libname, libpath, filename, import_method = get_asset_details_from_space(context, space, asset_type='MATERIAL', debug=True)
