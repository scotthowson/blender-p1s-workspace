...
class Fuse(bpy.types.Operator):
    ...
    def modal(self, context, event):
        ...
        if event.type in events:
            ...
            try:
                ret = self.fuse(self.active, modal=True)
    ...
    def fuse(self, active, modal=False):
        if self.segments > 0 or modal is True:
            ...
            if ret:
                ...
                if self.method == "FUSE":
                    ...
                    fuse_faces, _ = fuse_surface(bm, spline_sweeps, self.smooth, self.matindex, self.capholes, self.capdissolveangle, self.cyclic, debug=debug)
